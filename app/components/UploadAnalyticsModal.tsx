...
interface UploadAnalyticsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function UploadAnalyticsModal({ isOpen, onClose }: UploadAnalyticsModalProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    if (!selectedFile || !campaignName.trim()) {
      alert('Por favor, selecciona un archivo CSV y proporciona un nombre para la campaña');
    ...
    // The form will submit normally and Remix will process it
  };
      ...
      <div className={`absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200 flex-shrink-0">
            <h2 className="text-xl font-semibold text-gray-900">Subir Reporte de Analíticas</h2>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <Form id="upload-form" method="post" encType="multipart/form-data" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="text-sm font-medium text-blue-900 mb-2">Formato del CSV de Callbell</h4>
                <ul className="text-xs text-blue-700 space-y-1">
                  <li>• Cuatro columnas: phone_number, message_id, status, errors</li>
                  <li>• El archivo debe tener encabezado en la primera fila</li>
                  <li>• Status: delivered, failed, read</li>
                  <li>• Message ID: ID único del mensaje de Callbell</li>
                  <li>• Errors: JSON opcional con detalles del error</li>
                </ul>
                <div className="mt-2 p-2 bg-blue-100 rounded text-xs text-blue-800">
                  <strong>Ejemplo:</strong><br />
                  phone_number,message_id,status,errors<br />
                  +1234567890,msg_123,delivered,{}<br />
                  +1234567891,msg_124,failed,{`{"error": "Invalid number"}`}
                </div>
              </div>
